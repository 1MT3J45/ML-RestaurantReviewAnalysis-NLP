...
def streamers(full_dataset):
    ...
    stream1.to_csv('Wave4/stream1.csv', index=False)
    stream2.to_csv('Wave4/stream2.csv', index=False)
    stream3.to_csv('Wave4/stream3.csv', index=False)
...
def sheet_generator(s1, s2, s3):
    stream1 = s1  # Lemmas
    stream2 = s2  # Bi-grams
    stream3 = s3  # Dependency Relations

    df = pd.concat([stream1, stream3], axis=1)
    df.columns = ['lemmas', 'bigrams']
    df.to_csv('Wave4/FeatureSet.csv', index=False)
    df = pd.read_csv('Wave4/FeatureSet.csv', sep=',')
    ...
    whole_df = pd.concat([dataset.iloc[0:, 0], stream1, stream3, dataset.iloc[0:, 2]], axis=1)
    whole_df = whole_df.rename(columns={'text': 'reviews', 0: 'lemmas', 1: 'depnrel',
                                        'aspectCategories/aspectCategory/0/_category': 'aspectCategory'})
    whole_df.to_csv('Wave4/WholeSet.csv', index=False)
    whole_df = pd.read_csv('Wave4/WholeSet.csv', sep=',')
    # u_feat = list()
    # try:
...
def corrector(combined_features_list):
    ...
    pd.Series(ngram_list).to_csv('Wave4/ngram_list.csv', index=False)
    return ngram_list
...
def the_machine(X_train, X_test, y_train, y_test):
    ...
    prf_to_csv(prf, 'RandomForest_LD.csv')
    ...
    evaluator(prf, li2, total)
    prf_to_csv(prf, 'LinearSVC_LD.csv')
    ...
    for i in range(len(prf) - 1):
        ...
        print(
            '%s \t %.2f \t\t %.2f \t %.2f \t %.2f \t %.2f \t   %.1f' % (li[i], x[0], x[1], x[2], x[3], x[4], total[i]))
    evaluator(prf, li2, total)
    prf_to_csv(prf, 'MultinomialNB_LD.csv')
    ...
    for i in range(len(prf) - 1):
        ...
        print('%s \t %.2f \t\t %.2f \t %.2f \t %.2f \t %.2f \t   %.1f' % (li[i], x[0], x[1], x[2], x[3], x[4],
                                                                          total[i]))
    evaluator(prf, li2, total)
    prf_to_csv(prf, 'VotingClassifier_LD.csv')
...
\t Choice:"""))
        if choice == 1:
            ...
            a = pd.read_csv('Wave4/stream1.csv', header=None)
            b = pd.read_csv('Wave4/stream2.csv', header=None)
            c = pd.read_csv('Wave4/stream3.csv', header=None)
            ...
            max_feat = take_feat()
            df2 = pd.read_csv('Wave4/ngram_list.csv', header=None)
            ...
            #     X_train = np.array(pd.read_csv('Wave4/X_train.csv', header=None))
            #     X_test = np.array(pd.read_csv('Wave4/X_test.csv', header=None))
            #     y_train = np.array(pd.read_csv('Wave4/y_train.csv', header=None))
            #     y_test = np.array(pd.read_csv('Wave4/y_test.csv', header=None))
            #
            #     print('\t'*6,"SPLIT AVAILABLE & COMPLETE")
            # else:
            X_train, X_test, y_train, y_test = split_train_test(X, y)
            print("\t"*6, "TRAIN TEST SPLIT READY")
            # pd.DataFrame(X_train).to_csv('Wave4/X_train.csv', index=False)
            # pd.DataFrame(X_test).to_csv('Wave4/X_test.csv', index=False)
            # pd.DataFrame(y_train).to_csv('Wave4/y_train.csv', index=False)
            # pd.DataFrame(y_test).to_csv('Wave4/y_test.csv', index=False)
        elif choice == 5:
            try:
