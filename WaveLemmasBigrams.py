...
def streamers(full_dataset):
    ...
    stream1.to_csv('Wave3/stream1.csv', index=False)
    stream2.to_csv('Wave3/stream2.csv', index=False)
    stream3.to_csv('Wave3/stream3.csv', index=False)
...
def sheet_generator(s1, s2, s3):
    ...
    df = pd.concat([stream1, stream2], axis=1)
    df.columns = ['lemmas', 'bigrams']
    df.to_csv('Wave3/FeatureSet.csv', index=False)
    df = pd.read_csv('Wave3/FeatureSet.csv', sep=',')
    ...
    whole_df = pd.concat([dataset.iloc[0:, 0], stream1, stream2, dataset.iloc[0:, 2]], axis=1)
    whole_df = whole_df.rename(columns={'text': 'reviews', 0: 'lemmas', 1: 'bigrams',
                                        'aspectCategories/aspectCategory/0/_category': 'aspectCategory'})
    whole_df.to_csv('Wave3/WholeSet.csv', index=False)
    whole_df = pd.read_csv('Wave3/WholeSet.csv', sep=',')
    # u_feat = list()
    # try:
    ...
    Feature_df.to_csv('Wave3/Feature.csv', index=False)
    del whole_df,
...
def corrector(combined_features_list):
    ...
    pd.Series(ngram_list).to_csv('Wave3/ngram_list.csv', index=False)
    return ngram_list
...
\t Choice:"""))
        if choice == 1:
            ...
            a = pd.read_csv('Wave3/stream1.csv', header=None)
            b = pd.read_csv('Wave3/stream2.csv', header=None)
            c = pd.read_csv('Wave3/stream3.csv', header=None)
            ...
            max_feat = take_feat()
            df2 = pd.read_csv('Wave3/ngram_list.csv', header=None)
            ...
            #     X_train = np.array(pd.read_csv('Wave3/X_train.csv', header=None))
            #     X_test = np.array(pd.read_csv('Wave3/X_test.csv', header=None))
            #     y_train = np.array(pd.read_csv('Wave3/y_train.csv', header=None))
            #     y_test = np.array(pd.read_csv('Wave3/y_test.csv', header=None))
            #
            #     print('\t'*6,"SPLIT AVAILABLE & COMPLETE")
            # else:
            X_train, X_test, y_train, y_test = split_train_test(X, y)
            print("\t"*6, "TRAIN TEST SPLIT READY")
            # pd.DataFrame(X_train).to_csv('Wave3/X_train.csv', index=False)
            # pd.DataFrame(X_test).to_csv('Wave3/X_test.csv', index=False)
            # pd.DataFrame(y_train).to_csv('Wave3/y_train.csv', index=False)
            # pd.DataFrame(y_test).to_csv('Wave3/y_test.csv', index=False)
        elif choice == 5:
            try:
